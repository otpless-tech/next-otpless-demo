...
import { verifyCode, sendOTP, verifyOTP, resendOTP, magicLink, verifyToken } from 'otpless-next-js-auth-sdk';
const app = express();
...
app.get('/magic-link-token', async (req, res) => {
  try {
    const magicLinkTokens = await magicLink(phoneNumber,email,redirectURI,channel,clientId,clientSecret);
...
app.get('/verify-token', async (req, res) => {
    try {
        const userDetailUsingToken = await verifyToken(token,clientId,clientSecret);
...
app.get('/verify-code', async (req, res) => {
    try {
        const userDetailUsingCode = await verifyCode(code, clientId, clientSecret);
...
app.get('/send-OTP', async (req, res) => {
    try {
        const sendOTP = await sendOTP(phoneNumber, email, channel, hash, orderId, expiry, otpLength, clientId, clientSecret);
